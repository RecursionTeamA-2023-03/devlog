...
1. 進捗確認  
    - NestjsサーバーのAPI    
        認証(csrf-token・jwt-tokenの発行やsignup, login, logout)、プロジェクト追加・更新・削除、メンバー追加・削除、ダイアグラム追加・更新・削除のためのAPIを実装した。
    - 学習ページ    
        SSGによりNext.jsがpublicフォルダに保存された静的なmdファイルから学習ページを生成できるようにした。またmonacoエディタをplantUMLで使用できるように設定を行った。現在はシーケンス図のみ簡単なシンタックスハイライティングができるような設定なので今後追加が必要になる。
    - チートシートページ    
        学習ページ同様にSSGで静的にページを生成できるようにした。
2. 課題と方針   
    - plantUML画像生成のAPIをnestサーバーで実装する。
    - フロントエンドで認証用のAPIを使用して認証機能を実装する。
    - AWS ES２でバックエンドをDockerを用いてデプロイする。
3. 次回ミーティング   
4/5 20:00    
...
1. 進捗確認  
    - plantUML画像生成API    
        Nestjs上でAPIを作成したがnode-plantuml-backライブラリが実行できない。画像生成の各ストリーム処理をイベントエミッタ等でデバックしたが原因分からず。現状、URLにエンコードしたplantUMLシンタックスのデコードから画像データ生成へのストリームが何も出力していない。また、元々、Next.jsでも画像生成処理が非常に遅いといった課題もあったので今回の課題と含めて考慮して、plantUMLが公式に配布しているplantUML実行ファイルと同じくjavaで実行できるservletサーバーを画像生成用に別のコンテナとして用意する。
    - 認証機能    
        csrf-tokenを取得してCookieのヘッダーに追加する部分まで完成。jwtトークンを用いたログイン処理などは今後追加する。
    - AWSへのデプロイ    
        ES２インスタンスへのデプロイが完成。ドメインは現状なく固定IPアドレスでアクセス可能。無料枠での最小構成（t2.micro）。実運用に耐え得るスペックか疑問。また、Dockerを活用していることもありECS等の活用も考えたい。
2. 課題と方針   
    - plantUMLサーバーの構築とバックエンドとフロントエンドの統合
    - 認証情報のやり取り等でのHTTPS接続
    - CICDをどうするか、最適なデプロイ方法は何か。
3. 次回ミーティング   
4/8 20:00    
